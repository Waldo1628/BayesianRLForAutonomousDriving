...
preSortedDict = {}
directory = r'C:/projects/TacticalDM/logs/train_agent_20210714_215325/'
count = 0
for filename in os.listdir(directory):
    if filename.endswith("_0"):
        ...
        size = len(filename)
        mod_filename = filename[:size - 2]
        agent_name = mod_filename
        ...
        if case == 'rerun_test_scenarios':
            ...
            preSortedDict[int(mod_filename)] = float(action_info['coefficient_of_variation'][action])

# SORT BY KEY
preSortedDict = dict(sorted(preSortedDict.items()))
# HOW TO ITERATE THRU DICTIONARY
xArray=[]
yArray=[]

for k, v in preSortedDict.items():
    #print(k, v)
    xArray.append(k)
    yArray.append(v)
plt.figure(figsize=(10, 5))
plt.title("Figure 4, Uncertainty of Chosen Actions")
